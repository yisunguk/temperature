...
def extracted_edit_form(initial_date: str, initial_temp, initial_hum) -> Tuple[str, float, float]:
    st.subheader("추출 결과 확인/수정")
    col1, col2, col3 = st.columns(3)
    with col1:
        date_str = st.text_input("일자 (YYYY-MM-DD)", value=initial_date or "")
    with col2:
    ...
    return date_str, temp, hum

def table_view(df: pd.DataFrame):
    st.subheader("저장된 데이터")
    st.dataframe(df, use_container_width=False, width="stretch")  # deprecation 대응
